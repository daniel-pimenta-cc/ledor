...
- [x] Improve speed ramp-up (more natural curve? configurable duration/word count?)
...
- [x] Image support in books (inline figures from EPUB) — today only text is tokenized and the cover is extracted; inline images are dropped in `HtmlStripper`/tokenizer
- [ ] TTS Android: highlight por palavra só funciona com a Google TTS (única engine que emite `onRangeStart`; SherpaTTS e afins avançam por sentença). Follow-up possível: fallback de highlight estimado por timer (como o backend Linux) quando nenhum `onRangeStart` chega na primeira utterance. Ver seção "Limitations" em [docs/tts-mode.md](docs/tts-mode.md).
- [x] Testar de novo no Linux depois das correções da rodada Android (clamp de progresso sync, fixes de TTS) — validado 2026-07-07: TTS ok, sync ok; de quebra corrigida a sessão do Drive que não persistia entre aberturas (`prompt=consent`/`access_type=offline` injetados no `_launchPrompt`)
- [ ] Testar as builds de release do CI (tag no git) — inclui registrar o SHA-1 da release key no Google Cloud Console pro Sign-In funcionar na build assinada
- [ ] Testar no tablet
- [ ] Verificar que as stats (reading sessions) estão sendo registradas nos 3 modos de leitura
- [ ] UX: botão no fim do livro, visível em todos os modos, pra abrir a tela de completion/gerar a imagem (hoje só dispara organicamente no fim ou via FinishBookButton escondido no sheet/side panel)
- [ ] TTS Linux com Piper (via speech-dispatcher `sd_generic`): highlight ainda desincroniza levemente em frases longas com muita pontuação. Hoje o `SpeechdSocketBackend` estima a posição via timer adaptativo (baseline 150 WPM + medição empírica por voz + multiplicadores de dwell em `,`/`;`/`:`/`.`/`!`/`?`), porque `sd_generic` + Piper não expõe `INDEX_MARK` real. Refinos possíveis: (a) inserir `<mark>` SSML por palavra e pedir ao spd pra emitir INDEX_MARK reais (depende do backend suportar), (b) calibrar multiplicadores com base na duração medida vs estimada do segmento, (c) cortar utterances em sentenças menores no `extractSentenceFrom` pra reduzir a derivação acumulada.
